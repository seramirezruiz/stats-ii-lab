...
- Wald Estimator $LATE = \frac{con(Y_i,Z_i)}{cov(D_i,Z_i)}$. We can calculate this by hand. Let's try that now, using the values from the table above. We can also calculate the average malaria rates among those who did and did not receive an SMS (no SMS = 0.504, yes SMS = 0.18).
...
You can also do this with the sample code that Professors Munzert and Stoetzer utilized in their sessions. 
