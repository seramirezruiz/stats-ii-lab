---
title: "Research Discontinuity Design (RDD)"
...
```{r fig.align="center"}
ggplot(mlda, aes(x=agecell, y=treatment, color=factor(treatment))) +
  geom_point() + 
  labs(x= "Age", y = "Mortality rate from motor vehicle \naccidents (per 100,000)") +
  scale_color_discrete(name = " ", labels = c("Under legal drinking age", "Over legal drinking age")) +
...
As it was pointed out in the lecture, we can decide on a model that we believe reflects the relationship of our $E(Y_i|\tilde{X}_i)$:
...
<p>
We will show you how to visualize this with `ggplot`. You can find a base R `plot`alternative in the lecture code for this week's lecture.<p>

<center>**LET'S LOOK AT A SCATTERPLOT TO GET AN IDEA OF WHAT WE ARE DEALING WITH**</center>
<p>
```{r fig.align="center"}
ggplot(mlda, aes(x=agecell, y=outcome)) +
  geom_point() +
  geom_vline(xintercept = 21, linetype = "dotted") +
...
Let's run a linear model with common slopes and plot our results. <p>
...
```{r fig.align = 'center'}
mlda$yhat_linear <- predict(linear_common_slope) #we create a new variable containing the predicted years of schooling from the model

linear_plot <- mlda %>% ggplot(aes(x = agecell,  y = yhat_linear, col = as.factor(treatment))) +
  geom_point(aes(x = agecell, y = outcome, col = as.factor(treatment))) +
  labs(title = "Linear model with common slope",
       x = "Forcing variable (Age)",
       y = "Mortality rate from motor vehicle \naccidents (per 100,000)") +
  geom_line(data = mlda[mlda$agecell >= 21,], color = "#00BFC4", size = 1) +
  geom_line(data = mlda[mlda$agecell < 21,], color = "#F8766D", size = 1) +
...
Let's run the linear model to gather the slopes for both groups and plot our results. This is achieved by interacting our treatment and running variables<p>
...
diff_slopes_plot <- mlda %>% ggplot(aes(x = agecell,  y = outcome, col = as.factor(treatment))) +
  geom_point() +
...
```{r}
quadratic <- lm(outcome ~ agecell + I(agecell^2) + treatment + I(agecell*treatment) + I((agecell^2)*treatment), data = mlda)
summary(quadratic)
```
<center>**WHAT DO THESE RESULTS TELL US?** </center>
<p><p>
...
```{r fig.align = 'center'}
mlda$yhat_quadratic <- predict(quadratic) #we create a new variable containing the predicted years of schooling from the model

quadratic_plot <- mlda %>% ggplot(aes(x = agecell,  y = yhat_quadratic, col = as.factor(treatment))) +
  geom_point(aes(x = agecell, y = outcome, col = as.factor(treatment))) +
  labs(title = "Linear model with common slope",
       x = "Forcing variable (Age)",
       y = "Mortality rate from motor vehicle \naccidents (per 100,000)") +
...
RDestimate() is part of the `rdd` package. It performs local linear regressions to either side of the cutpoint using the Imbens-Kalyanaraman optimal bandwidth calculation. The syntax is very similar to the one we are familiar:
...
ggplot(cct_data, aes(x = hh_income, y = treatment, color = factor(treatment))) + 
  geom_point() + 
  labs(x= "Household Income", y = "Treatment") +
  scale_color_discrete(name = " ", labels = c("No treatment", "Treatment")) +
  geom_vline(xintercept = 20000, linetype = "dotted") +
  theme_minimal()
```
